...
def main():
    ...
    for tag in path_dict.keys():
        ...
        INFO_PKL = "/media/franco/hdd/matteogombia/OpenPCDet/data/kitti/kitti_infos_val.pkl"

        # ==========================================
        # LOAD DETECTIONS (new_dt)
        # ==========================================
        with open(result_path / 'result.pkl', 'rb') as f:
            # OpenPCDet's result is ALREADY a flat list! No loops needed.
            new_dt = pickle.load(f)  

        # ==========================================
        # LOAD GROUND TRUTH (new_gt)
        # ==========================================
        with open(INFO_PKL, 'rb') as f:
            kitti_infos = pickle.load(f)

        # We need to make sure the Ground Truth list exactly matches the 
        # order of the Predictions list based on the frame_id.
        new_gt = []

        # Create a quick dictionary to look up ground truth by frame_id
        gt_lookup = {info['point_cloud']['lidar_idx']: info['annos'] for info in kitti_infos}

        # Loop through our predictions to build the perfectly matched GT list
        for dt_frame in new_dt:
            frame_id = dt_frame['frame_id']
            
            if frame_id in gt_lookup:
                # Append the annotations (ground truth boxes, names, etc.)
                new_gt.append(gt_lookup[frame_id])
            else:
                print(f"Warning: Frame {frame_id} missing from Ground Truth!")

        # ==========================================
        # VERIFY
        # ==========================================
        print(f"Loaded {len(new_dt)} predicted frames (dt)")
        print(f"Loaded {len(new_gt)} ground truth frames (gt)")

        current_classes = [0, 1, 2] # Car, Pedestrian, Cyclist
